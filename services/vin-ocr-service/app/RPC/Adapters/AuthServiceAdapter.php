...
/**
 * AuthServiceAdapter for VIN OCR Service
 * 
...
class AuthServiceAdapter
{
    ...
    /**
     * Verify user permissions for VIN OCR operations
     */
    public function verifyVinOcrPermissions(int $userId): bool
    {
        $startTime = microtime(true);
        $correlationId = request()->header('X-Correlation-ID', uniqid('rpc_', true));
        
        try {
            $this->logRpcCall('verifyVinOcrPermissions', ['user_id' => $userId], $correlationId);
            
            $response = $this->authRpc->call('auth.verifyPermissions', [
                'user_id' => $userId,
                'permissions' => ['vin_ocr.process', 'vin_ocr.read']
            ]);
            
            $duration = round((microtime(true) - $startTime) * 1000, 2);
            $this->logRpcCall('verifyVinOcrPermissions', ['duration_ms' => $duration], $correlationId, 'success');
            
            return isset($response['success']) && $response['success'] && ($response['data']['authorized'] ?? false);
        } catch (Exception $e) {
            $duration = round((microtime(true) - $startTime) * 1000, 2);
            $this->logRpcError('verifyVinOcrPermissions', $e, $correlationId, $duration);
            return false;
        }
    }
