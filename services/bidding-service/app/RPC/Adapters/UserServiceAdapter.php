...
class UserServiceAdapter
{
    ...
    public function reserveFunds(int $userId, float $amount, string $bidId): bool
    {
        try {
            ...
            $response = $this->userRpcClient->call('user.reserveFunds', [
                'user_id' => $userId,
                'amount' => $amount,
                'reference' => $bidId,
                'type' => 'bid_reserve',
            ...
            $duration = microtime(true) - $startTime;
            $this->logRpcCall('reserveFunds', compact('userId', 'amount', 'bidId'), $response, $duration);

            return $response['success'] ?? false;
        } catch (\Exception $e) {
            $this->logRpcError('reserveFunds', compact('userId', 'amount', 'bidId'), $e);
            return false;
        }
    }
    ...
    public function releaseFunds(int $userId, string $bidId): bool
    {
        try {
            ...
            $response = $this->userRpcClient->call('user.releaseFunds', [
                'user_id' => $userId,
                'reference' => $bidId,
                'source' => 'bidding_service'
            ]);
            ...
            $duration = microtime(true) - $startTime;
            $this->logRpcCall('releaseFunds', compact('userId', 'bidId'), $response, $duration);

            return $response['success'] ?? false;
        } catch (\Exception $e) {
            $this->logRpcError('releaseFunds', compact('userId', 'bidId'), $e);
            return false;
        }
    }
    ...
    public function getUserBiddingPreferences(int $userId): array
    {
        try {
            ...
            $response = $this->userRpcClient->call('user.getBiddingPreferences', [
                'user_id' => $userId
            ]);

            $duration = microtime(true) - $startTime;
            $this->logRpcCall('getUserBiddingPreferences', compact('userId'), $response, $duration);

            return $response['success'] ?? false ? $response['data']['preferences'] ?? [] : [];
        } catch (\Exception $e) {
            $this->logRpcError('getUserBiddingPreferences', compact('userId'), $e);
            return [];
        }
    }
