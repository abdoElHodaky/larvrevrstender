...
/**
 * AuthServiceAdapter for Order Service
 * 
...
class AuthServiceAdapter
{
     ...
     */
    public function verifyOrderPermissions(int $userId, string $operation): bool
    {
        $startTime = microtime(true);
        $correlationId = request()->header('X-Correlation-ID', uniqid('rpc_', true));
        
        try {
            $this->logRpcCall('verifyOrderPermissions', ['user_id' => $userId, 'operation' => $operation], $correlationId);
            ...
            $duration = round((microtime(true) - $startTime) * 1000, 2);
            $this->logRpcCall('verifyOrderPermissions', ['duration_ms' => $duration], $correlationId, 'success');
            
            return isset($response['success']) && $response['success'] && ($response['data']['authorized'] ?? false);
        } catch (Exception $e) {
            $duration = round((microtime(true) - $startTime) * 1000, 2);
            $this->logRpcError('verifyOrderPermissions', $e, $correlationId, $duration);
            return false;
        }
    }
